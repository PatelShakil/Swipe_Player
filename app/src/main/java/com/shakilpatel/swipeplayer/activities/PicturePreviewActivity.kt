...
class PicturePreviewActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val captureResolution = findViewById<TextView>(R.id.nativeCaptureResolution)
        val captureLatency = findViewById<TextView>(R.id.captureLatency)
        val exifRotation = findViewById<TextView>(R.id.exifRotation)

        val delay = intent.getLongExtra("delay", 0)
        val ratio = AspectRatio.of(result.size)
        captureLatency.setText("Approx. latency : " +  "$delay milliseconds")
        captureResolution.setText("Resolution : " +  "${result.size} ($ratio)")
        exifRotation.setText("EXIF rotation : "+ result.rotation.toString())
